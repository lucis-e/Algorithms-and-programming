...
```
# 0.3/1 The '^' operator in your pattern only applies to the letter S and the '.' character is not being escaped.
# In the case the perl functionalities allow you to do something like: ^(?!.*Sep\\.)
...
grep2b<-"^[[:alnum:]]{3}[^aAzZ]"
```
# # 0.5/1 Missing last contidion, and it finds any character not just letters that are not one of the ones listed. Something like [B-y] works :) 
...
```
# 1/1 "GRP1GRP[12]GRP2GRP[12]" works too 
...
```
# 3/4 Correct calculations but that's a distribution plot, no the frequences 'along' the contig
...
RSCU
```
# 4/5 It hurts to not give you 5/5 because that's a very elegant/simple way to calculate it. But you counted all trinucleoties in all reading frames, not codons. trinucleotideFrequency(five_p, step = 3) would do it. 
...
dotPlot(sponge_cds$Aqu2.1.44141, 100, 40)
```
# 4/8 Close but it seems like it's not really matching everything against everything?. Also, It would be very much appreciated if next time you write a little documentation on your function, specially when you have nested operations. Everyone, specially your future self, will like that :p 
...
```
# 2/3 Yeah you get the idea but without a proper dotplot this figure doesn't make sense
...
dotPlotTwo(sponge_cds$Aqu2.1.44141,sponge_cds$Aqu2.1.19739,100,10)

```
# 1/4 Did you make this work? 