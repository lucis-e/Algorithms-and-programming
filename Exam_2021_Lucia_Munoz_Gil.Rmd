...
```{r}
seq<-unlist(readDNAStringSet("ftp://ftp.sanger.ac.uk/pub/project/pathogens/sa/MRSA252.dna"))   ### not a good idea to call a variable "seq" because that's already a function name - it will rarely cause an actual problem but you will still have to be extra-careful because of it, and it will confuse other readers of your code
seq<-seq[1:5000]
...
my.kmer.coverage<-function(sequence, kLen, cov){
  ...
  start_pos<-sample(1:(length(sequence)-max(read_length)), read_num, replace = T)  ### nice reasoning for this!
  read_coordinates<-IRanges(start = start_pos, width = read_length)
  reads<-extractAt(sequence, read_coordinates)   ### best possible choice of function to do this
...
mismatch<-unlist(unlist(DNAStringSetList(lapply(char, function(x) sample(c(x, DNA_BASES[DNA_BASES!=x]), replace = T, 1, prob = c(0.997, 0.001, 0.001, 0.001))))))   ### creative & smart approach - really nice usage of sample()!
  ...
  kmers<-extractAt(mismatch, kmer_coord)      ### very smart approach, exceptionally well planned, and super-fast. Wnonderful!!
  ...
  kmers_dt<-as.data.table(as.data.frame(kmers))    ### could've been directly as.data.table, you actually don't need as.data.frame
...
*Again excellent code and excellent explanation as well. Regarding your last comment: what actually happened is that 5% is unusually high for heterozigosity in natural conditions, and we have a pretty short sequence, so the combination of those two factors skews the plot here. But this kind of plot with real-life data would look exactly as you describe. Great thinking!*

*And also, thank you for commenting your code so thoroughly, it's been a breeze to evaluate so far :)*
...
``` 
*Perfect. Full points for the task.*
...
###
My.k.means(iris[1:2], 3)

##
clusters_dt; new_clusters_dt
```

<span style="color:red"> 14/15 Great! Only you had to run the process of generating centroids several times in order to obtain multiple centroids and calculate mean of them to get the consensus </span>
...
fastq<-fread("https://d28rh4a8wq0iu5.cloudfront.net/ads1/data/ERR037900_1.first1000.fastq")
