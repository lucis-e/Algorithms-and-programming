...
```
<span style="color:blue"> 1/1 It is easier to only subset the ONE desired column. </span>
...
```
<span style="color:red"> 0/2  flights[,.(numberofmaxflights=sum(max(air_time)==air_time)),by=carrier] </span>
...
```
<span style="color:red"> 1/2 What is vectorization in R? Sapply add additional complicated layer that is not required. The same function logic as Aitana , as well as the format of code where you define a variable and then call it using  </span>
...
```
<span style="color:blue"> 1/1  Poorly made functions leads to running this part of the code for too long. </span>
...
```
<span style="color:purple"> 2/2  Poorly made functions leads to running this part of the code for too long. </span>
...
```
<span style="color:red"> 0.75/1 Good thing you checked if they are there but you still need to specify it in your code.  Missing: (origin == "JFK" & dest == "MIA") | (origin == "MIA" & dest == "JFK") </span>
...
```
<span style="color:red"> 0/1 pts uniqueN does the counting of unique values. Here you had to found the shortest median lengths. Here you are calculating median of 1 unique value for each sponge. Correct solution is unique(spongeDiamond[,.(sponge, qseqid, qlen)])[,.(median(qlen)), sponge][V1==min(V1)] or some other variation of filtering only unique values and then performing median calculation. </span>
...
```
<span style="color:red"> 0.5/1 pts. This syntax does not work the way you think. First you find the max/min values for each sponge correctly and then you subset by those number of max/min mismatches. Sign = is assignin calculation to a variable while == is for comparing. You could have rewrite this part as [, c(qseqid[total_mismatch == max(total_mismatch)], qseqid[total_mismatch == min(total_mismatch)]), by=sponge] but you would then have multiple solutions for lowest values. If you did that then it would be fine but this is the most efficient way to do this in data.table: spongeDiamond[,.(sum_contig=sum(mismatch)), .(qseqid, sponge)][order(sponge, -sum_contig), .SD[c(1,.N)], sponge]  </span>
...
```
 <span style="color:green"> 3/2 pts Perfect :D (one extra point)  </span>
...
percent<-function(v){
  ...
  ## Correction by PS
  res<-paste(paste(ifelse(first==10, 9, first), 0, sep = ""), paste(ifelse(first==10, 9, first)+1, 0, "%", sep = ""), sep = "-")
  res
}
...
<span style="color:red"> 2/3 pts  In your function you are using the ifelse in the "air", you have to save those results in a variable "first" or a new one when working in R. See the change in the code.  </span>
...
```
<span style="color:blue"> 2/2 pts </span>  
